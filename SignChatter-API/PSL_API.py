...
#            intermediateFrame = cv2.warpAffine(frame, rotmat, dim)
            ...
            # cropping
            size = frame.shape
            finalFrame = frame[80:(size[0] - 160), 0:(size[1] - 20)]
#            cv2.imwrite('frame.jpg',finalFrame)
...
model_path = r"/root/final_year_project/Mediapipe-API-PSL/PSL_15C_45F_64B_20T_.h5"
...
async def predict_action(file: UploadFile = File()):
    video_path = f"/root/final_year_project/Mediapipe-API-PSL/videos/{file.filename}"
